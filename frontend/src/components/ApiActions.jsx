...
// e.g. current=8, total=42 -> [1, "…", 7, 8, 9, "…", 42]
...
export default function ApiActions({
  ...
  const totalRecipes =
    result?.type === "recipes" ? result.data.recipes.length : 0;
  const totalPages = Math.max(1, Math.ceil(totalRecipes / PAGE_SIZE));
  const pageRecipes =
    result?.type === "recipes"
      ? result.data.recipes.slice(
          (recipesPage - 1) * PAGE_SIZE,
          recipesPage * PAGE_SIZE,
        )
      : [];
  ...
  return (
    <div>
      ...
      {result?.type === "insights" && !errorMsg && (
        <div className="mt-4 bg-white p-4 shadow-lg rounded-lg border border-gray-100 text-sm text-gray-600">
          ...
          <span className="font-semibold">
            {DIET_LABELS[selectedDiet] ?? selectedDiet}
          </span>{" "}
          ...
          {totalPages > 1 && (
            <div className="flex flex-col items-center gap-2 mt-4">
              <div className="flex justify-center flex-wrap gap-2">
                ...
                {pageWindow(recipesPage, totalPages).map((p) =>
                  typeof p === "number" ? (
                    <button
                      key={p}
                      type="button"
                    ...
                    <span key={p} className="px-1 py-1 text-sm text-gray-400">
                      …
                    </span>
                  ),
                )}
                <button
                  type="button"
                  onClick={() =>
                    setRecipesPage((p) => Math.min(totalPages, p + 1))
                  }
                ...
                {recipesPage < totalPages - 1 && (
                  <button
                    type="button"
                    onClick={() => setRecipesPage(totalPages)}
                    className="px-3 py-1 bg-gray-300 rounded hover:bg-gray-400 text-sm"
                  >
                    Last
                  </button>
              ...
              <p className="text-xs text-gray-500">
                Page {recipesPage} of {totalPages}
              </p>
            </div>
          )}
        </div>
      ...
      {result?.type === "clusters" && !errorMsg && (
        <div className="mt-4">
          <p className="text-sm text-gray-600 mb-3">
            {result.data.clusters.length} cluster
            {result.data.clusters.length === 1 ? "" : "s"} for{" "}
            <span className="font-semibold">
              {DIET_LABELS[selectedDiet] ?? selectedDiet}
            </span>{" "}
            ({result.data.meta.executionTimeMs} ms)
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {result.data.clusters.map((cluster) => (
              <div
                key={cluster.id}
                className="bg-white p-4 shadow-lg rounded-lg border border-gray-100"
              >
                <h4 className="font-semibold text-gray-900 text-sm">
                  {cluster.label}
                </h4>
                <p className="text-xs text-gray-500 mb-2">
                  {cluster.recipeCount} recipes
                </p>
                <p className="text-xs text-gray-600">
                  Protein: {cluster.avgProtein}g
                </p>
                <p className="text-xs text-gray-600">
                  Carbs: {cluster.avgCarbs}g
                </p>
